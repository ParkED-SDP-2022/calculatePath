...
class Boundaries:
    ...
    def __init__(self, fname):
        ...
        self.boundary_buffer = self.boundary_polygon.buffer(-self.robot_size_in_coords/4, single_sided=False)

        # positive distance is dilation which is used for each obstacle
        self.obstacle_buffer = MultiPolygon([obstacle.buffer(self.robot_size_in_coords/4, single_sided=False) for obstacle in self.obstacle_polygon.geoms])
